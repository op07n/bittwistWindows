    ...
    while ((c = getopt(argc, argv, "I:O:L:X:CM:D:R:S:T:F:h")) != -1) {
        switch (c) {
            ...
            case 'S':
                ...
                break;
            case 'T':
            if (_stricmp(optarg, "eth") == 0)
                header_opt = ETH;
            else if (_stricmp(optarg, "arp") == 0)
                header_opt = ARP;
            else if (_stricmp(optarg, "ip") == 0)
                header_opt = IP;
            else if (_stricmp(optarg, "icmp") == 0)
                header_opt = ICMP;
            else if (_stricmp(optarg, "tcp") == 0)
                header_opt = TCP;
            else if (_stricmp(optarg, "udp") == 0)
                header_opt = UDP;
            else
